 ...
 * representation of the status. A more detailed description is available via
 * <code>getDescription</code> method.
 * <p>
 * Status of a newly created order is set to <code>NEW</code> by the
 * constructor. All other statuses should be modified by the trade executor
 * only.
 * 
...
public enum OrderStatus {
	/**
	 * Order is created and ready for sending. All newly created orders must
	 * have a <code>NEW</code> status.
	 */
	...
	CANCELLATION_PENDING("cancelation pending",
			"Order cancellation is sent but not yet confirmed "
					+ "by the executor."),
	 ...
	 * 
	 * @param title
	 *            a human readable status title. This title is returned by
	 *            <code>toString</code> method. Actual field names, like
	 *            <code>PENDING</code> or <code>PARTIALLY_FILLED</code> should
	 *            not be displayed to the user but can be used for persistence
