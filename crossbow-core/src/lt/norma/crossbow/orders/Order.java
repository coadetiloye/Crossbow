...
/**
 * Base class for all order types. Extend this class to create new order types.
 * Use attributes to attach additional instructions to the order.
 * <p>
 * Most fields are final. To modify an order, cancel it and resubmit a new order
...
public abstract class Order {
	...
	/**
	 * Type of the order. Used only to provide human readable type of the order.
	 * Internally order types are identified by class and attributes.
	 */
	...
	/**
	 * Carries additional information about this order. Make sure the order
	 * executor knows how to interpret the added attributes.
	 */
	 ...
	 */
	public Order(long id, Contract contract, String type,
		...
		if (type == null || type.isEmpty()) {
			throw new OrderException("Type of the order for contract '"
					+ contract.toString() + "' is not set.");
		}
		if (direction == null || direction != OrderDirection.BUY
				&& direction != OrderDirection.SELL) {
			throw new OrderException("Order for contract '"
					+ contract.toString() + "' has invalid direction '"
					+ String.valueOf(direction) + "'.");
		}
		if (size <= 0) {
			throw new OrderException("Order for contract '"
					+ contract.toString() + "' has invalid size '" + size
					+ "'.");
		}
	...
	public String toString() {
		return (status + " " + type + " order to " + direction + " " + size
	...
	/**
	 * Get time when the order was submitted. Can be null if the order is not
	 * submited yet.
	 * 
	 * @return submit time
	 */
	 ...
	 * Get order size.
	 * 
	 * @return size of an order. Negative value means short trade, positive -
	 *         long
	 */
	public int getSize() {
		return size;
