...
public class Exchange implements Comparable<Exchange> {
	 ...
	 */
	public Exchange(String name, DateTimeZone timeZone,
			LocalTime tradingStarts, LocalTime tradingEnds) {
		// Data control
		if (name == null) {
	...
	/**
	 * Creates an exchange with time zone set to "America/New_York" and trading
	 * hours 09:30 - 16:00.
	 * 
	...
	public static Exchange createNyseExchange() {
		return new Exchange("Nyse", DateTimeZone.forID("America/New_York"),
				new LocalTime(9, 30, 0), new LocalTime(16, 0, 0));
	}
	 ...
	 * @param time
	 *            time to be checked
	 * @return true if the specified time equals to or is after trading start
	 *         time, and is before trading end time.
	 */
	...
	/**
	 * Checks if the exchange is equal to this exchange. Exchanges with the same
	 * name are equal.
	 * 
	 * @param object
	...
	/**
	 * Alphabetically compares name of this exchange to the other exchange's
	 * name.
	 * 
	 * @param exchange
	 *            exchange to be compared to this exchange
	 * @return 0 if names of both exchanges are the same; a negative integer if
	 *         name of this exchange is "less" than that of the other exchange;
	 *         a positive integer if name of this exchange is "greater";
	 */
	@Override
