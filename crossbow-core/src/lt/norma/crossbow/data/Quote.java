...
public class Quote {
	 ...
	 *            time of the quote
	 */
	public Quote(Contract contract, BigDecimal askPrice, int askSize,
			BigDecimal bidPrice, int bidSize, DateTime time) {
		if (contract == null) {
			throw new InvalidArgumentRuntimeException("contract", String
					.valueOf(contract));
		}
		if (askPrice == null) {
			throw new InvalidArgumentRuntimeException("askPrice", String
					.valueOf(askPrice));
		}
		if (bidPrice == null) {
			throw new InvalidArgumentRuntimeException("bidPrice", String
					.valueOf(bidPrice));
		}
		if (time == null) {
	 ...
	 * 
	 * @return size of spread. Value may be negative in case of a crossed
	 *         market.
	 */
	...
	public String toString() {
		return contract.toString() + "  ask: " + askSize + " @ "
				+ contract.getCurrency().formatNumber(askPrice) + "  bid: "
				+ bidSize + " @ "
				+ contract.getCurrency().formatNumber(bidPrice) + "  ["
