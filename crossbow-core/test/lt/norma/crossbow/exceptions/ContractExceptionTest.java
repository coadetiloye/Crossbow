...
public class ContractExceptionTest {
	...
	public void testCreation1() throws ContractException {
		...
		assertEquals(new StockContract("MSFT", Exchange.createNyseExchange(),
				Currency.createEur()), ce.getContract());
		assertEquals(cause, ce.getCause());
		assertEquals("Contract MSFT is invalid. Test exception.", ce
				.getMessage());
		ContractException ce2 = new ContractException(null, "Test exception.",
				cause);
		assertNull(ce2.getContract());

		ContractException ce3 = new ContractException(c, "Test exception.",
				null);
